                        ...
                        <div class="widget-body no-padding">
                            <div id="datatable_fixed_column_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                                <table id="datatable_fixed_column" class="table table-striped table-bordered dataTable no-footer" width="100%" role="grid" aria-describedby="datatable_fixed_column_info" style="width: 100%;">
                                    <thead>
                                    <!-- Create new role -->
                                    <form method="POST" action="{{route('backend.role.store')}}" id="smart-form-register" class="smart-form client-form">
                                        {{ csrf_field() }}
                                        <tr role="row">
                                            <th class="hasinput" rowspan="1" colspan="1">
                                                <input name="name" type="text" class="form-control" placeholder="{{__('backend.name')}}">
                                            </th>
                                            <th class="hasinput"  rowspan="1" colspan="1">
                                                <input name="display_name" type="text" class="form-control" placeholder="{{__('backend.display_name')}}">
                                            </th>
                                            <th class="hasinput" rowspan="1" colspan="1">
                                                <input name="description" type="text" class="form-control" placeholder="{{__('backend.description')}}">
                                            </th>
                                            <th class="hasinput"  rowspan="1" colspan="1">
                                                <button type="submit" class="btn btn-primary" href="javascript:void(0);">{{__('backend.add')}}</button>
                                                <button type="reset" class="btn btn-default" href="javascript:void(0);">Reset</button>
                                            </th>
                                        </tr>
                                        <tr role="row">
                                            <th data-class="expand" class="expand sorting_asc" tabindex="0" aria-controls="datatable_fixed_column" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending" style="width: 181px;">{{__('backend.name')}}</th>
                                            <th class="sorting" tabindex="0" aria-controls="datatable_fixed_column" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 193px;">{{__('backend.display_name')}}</th>
                                            <th data-hide="phone" class="sorting" tabindex="0" aria-controls="datatable_fixed_column" rowspan="1" colspan="1" aria-label="Age: activate to sort column ascending" style="width: 181px;">{{__('backend.description')}}</th>
                                            <th data-hide="phone" class="sorting" tabindex="0" aria-controls="datatable_fixed_column" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 169px;">{{__('backend.action')}}</th>
                                        </tr>
                                    </form>
                                    ...
                                    @foreach($roles as $role)
                                        <tr  role="row"  class="item-{{$role->id}}">
                                            ...
                                            <td class="text-center">
                                                <a href="{{route('backend.role.attachPerms',$role)}}" class="btn btn-warning btn-sm">
                                                    <i class="fa fa-gear fa-spin"></i>
                                                    {{__('backend.function')}}
                                                </a>
                                                <a href="{{route('backend.role.edit',$role)}}" class="btn btn-info btn-sm">
