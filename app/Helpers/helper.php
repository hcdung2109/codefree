...
class Helper
{
     ...
     * @param int $parent_id
     */
    public static function createHtmlNavigation($collections, $parent_id = 0, $tag_open = "<ul>", $tag_close = '</ul>')
    {
        foreach ($collections as $item) {
            // before show item as parent
            if ($item->parent_id == $parent_id) {
                echo '<li><a href="'.route('blog.list-article-of-category', ['slug_category' => $item->slug ]).'">'.$item->name.'</a>'.$tag_open;
                    self::createHtmlNavigation($collections, $item->id, '','');
                echo $tag_close.'</li>';
            }
        }
    }
    ...
    public static function treeviewCollections($data = array(),$parent_id = 0)
    {
        ...
        foreach($data as $val) {
            if ($val['parent_id'] == $parent_id) {
                echo '<li><a href="'.route('backend.category.edit',['id' => $val['id']]).'"><span><i class="fa fa-lg fa-plus-circle"></i> '.$val['name'].'</span></a><ul>';
